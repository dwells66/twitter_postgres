...
for file in $files; do
    ...
    python3 load_tweets.py --db "postgresql://postgres:pass@pg_normalized:1112" --inputs "$file"
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalized
    zcat $file | sed 's/\\u0000//g' | psql "postgresql://postgres:pass@pg_denormalized:1113" -c "COPY tweets_jsonb FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
